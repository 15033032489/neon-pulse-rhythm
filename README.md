...
Neon Pulse 是一款可直接在浏览器运行的四键下落式音乐节奏游戏。项目使用 React、TypeScript、Vite、Web Audio API 和 Vitest；演示音乐完全由浏览器实时合成，不加载外部歌曲、图片或字体。
...
打开终端显示的地址，默认是 `http://127.0.0.1:5173/`。四条轨道依次使用 `D`、`F`、`J`、`K`，也支持鼠标点击与手机多点触控。`Esc` 暂停；全屏只能由玩家主动点击界面按钮进入。
...
- 每张谱面按计分单位标准化为满分 1,000,000；Hold 头部与尾部各占一个计分单位。

正 Offset 会让谱面和判定相对音乐更晚，负 Offset 会让它们更早。手动滑块和自动校准结果都以 5ms 为步长。自动校准包含 4 个预热拍和 16 个采样拍，过滤明显异常值后使用中位数与 MAD 进行稳健估计。

设置以带版本号的结构保存在 `neon-pulse:settings`，并兼容旧的速度/Offset 设置。成绩按“歌曲 + 难度”保存在 `neon-pulse:records`，记录最高分、准确率、最大连击、最佳评级、FC 与 AP。放弃本局不会保存成绩，也不会把剩余音符强制补记为 Miss。
...
1. 在 `src/songs/catalog.ts` 增加歌曲元数据。`id` 必须唯一，`duration` 以秒为单位，当前内置合成配置为 `synthProfile: "chromatic"`。
